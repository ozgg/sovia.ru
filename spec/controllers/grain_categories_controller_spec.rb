...
RSpec.describe GrainCategoriesController, type: :controller do
  ...
  let(:valid_creation_parameters) { { grain_category: attributes_for(:grain_category) } }
  let(:valid_parameters) { { grain_category: { name: 'Changed' } } }
  let(:invalid_parameters) { { grain_category: { name: ' ' } } }
  ...
  shared_examples 'forbidden_editing' do
    ...
    it 'redirects to entity administration page' do
      expect(response).to redirect_to(admin_grain_category_path(entity))
    end
  end
    ...
    context 'when parameters are valid' do
      ...
      context 'authorization and redirects' do
        ...
        it 'redirects to created entity' do
          expect(response).to redirect_to(admin_grain_category_path(entity.class.last))
        end
      end
    end
  ...
  describe 'delete destroy' do
    ...
    context 'when entity is not locked' do
      ...
      it 'redirects to entities page' do
        expect(response).to redirect_to(admin_grain_categories_path)
      end
    end
