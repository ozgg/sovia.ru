...
RSpec.describe Api::WordsController, type: :controller do
  ...
  describe 'post toggle' do
    before(:each) { post :toggle, params: { id: entity, parameter: :processed } }
    ...
    context 'when entity is not locked' do
      ...
      it 'toggles parameters' do
        entity.reload
        expect(entity).to be_processed
      end
    end

    context 'when entity is locked' do
      let(:entity) { create :word, locked: true }
      ...
      it 'does not toggle parameter' do
        entity.reload
        expect(entity).not_to be_processed
      end
    end
  end
